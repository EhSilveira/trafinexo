...
export async function signUp({name,email,password,legalAccepted=false}){if(!legalAccepted)throw new Error("Aceite os Termos de Uso e a Política de Privacidade para continuar.");const normalized=String(email||"").trim().toLowerCase();const r=await fetch(`${SUPABASE_URL}/functions/v1/factory-info-suite-trial-signup`,{method:"POST",headers:{apikey:SUPABASE_KEY,"Content-Type":"application/json"},body:JSON.stringify({mode:"account_signup",product_slug:"trafinexo",name,email:normalized,password,legal_version:LEGAL_VERSION,metadata:{trial_days:15,terms_accepted:true,privacy_accepted:true,legal_accepted_at:new Date().toISOString()}})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data?.error==="email_already_registered"?"Este e-mail já está cadastrado.":data?.error||"Não foi possível criar a conta");if(!data?.session?.access_token||!data?.session?.refresh_token)throw new Error("A conta foi criada, mas a sessão não iniciou.");saveSession({session:{...data.session,user:data.user},user:data.user});await ensureWorkspace();return {session:{...data.session,user:data.user},user:data.user,access_token:data.session.access_token,refresh_token:data.session.refresh_token};}
