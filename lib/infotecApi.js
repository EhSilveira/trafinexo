...
const DEDICATED_SUPABASE_URL="https://vvjngcsftiffdsjwpjct.supabase.co";
const DEDICATED_SUPABASE_KEY="sb_publishable_KtLXC6mYAt4lTqrJvpl7jA_sgN1Spec";

export const USING_DEDICATED_SUPABASE=true;
export const SUPABASE_URL=DEDICATED_SUPABASE_URL;
export const SUPABASE_KEY=DEDICATED_SUPABASE_KEY;
...
export async function signUp({name,email,password,legalAccepted=false}){if(!legalAccepted)throw new Error("Aceite os Termos de Uso e a Política de Privacidade para continuar.");const normalized=String(email||"").trim().toLowerCase();const r=await fetch(`${SUPABASE_URL}/functions/v1/factory-info-suite-trial-signup`,{method:"POST",headers:{apikey:SUPABASE_KEY,"Content-Type":"application/json"},body:JSON.stringify({mode:"account_signup",product_slug:"trafinexo",name:String(name||"").trim(),email:normalized,password,legalAccepted:true,legal_version:LEGAL_VERSION})});const data=await r.json().catch(()=>({}));if(!r.ok||!data?.ok||!data?.session?.access_token||!data?.session?.refresh_token)throw new Error(data?.error==="email_already_registered"?"Este e-mail já está cadastrado. Entre com sua senha ou recupere o acesso.":data?.error||"Não foi possível criar a conta");saveSession({session:data.session,user:data.user});await ensureWorkspace();await fetch(`${SUPABASE_URL}/functions/v1/useinfotec-request-email-verification`,{method:"POST",headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${data.session.access_token}`,"Content-Type":"application/json"},body:JSON.stringify({product_slug:"trafinexo",email:normalized})}).catch(()=>null);return {session:data.session,user:data.user,access_token:data.session.access_token};}
