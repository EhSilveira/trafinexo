...
export default function EmailVerificationBanner({productSlug}){
  ...
  const load=useCallback(async()=>{
    ...
    if(!token||!userId){setVerified(null);return}
    const {url,key}=project(productSlug);
    try{
      const r=await fetch(`${url}/rest/v1/useinfotec_email_verification_status?select=verified_at&user_id=eq.${encodeURIComponent(userId)}&limit=1`,{headers:{apikey:key,Authorization:`Bearer ${token}`},cache:"no-store"});
      if(!r.ok){setVerified(null);return}
  ...
  async function requestVerification(){
    ...
    try{
      const r=await fetch(`${url}/functions/v1/useinfotec-request-email-verification`,{method:"POST",headers:{apikey:key,Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({product_slug:productSlug})});
  ...
  return <aside style={{margin:"12px 0 18px",padding:"14px 16px",border:"1px solid #cbdcf0",borderRadius:14,background:"#f4f8fc",display:"flex",gap:14,alignItems:"center",justifyContent:"space-between",flexWrap:"wrap"}}>
    ...
    <button type="button" onClick={requestVerification} disabled={busy} style={{border:0,borderRadius:999,padding:"10px 15px",background:"#071A33",color:"#fff",fontWeight:800,cursor:busy?"wait":"pointer"}}>{busy?"Enviando...":"Enviar confirmação"}</button>
  </aside>
}
