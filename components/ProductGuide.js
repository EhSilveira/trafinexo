...
const steps=[
 ...
 {selector:'.infotec-pwa-install',title:"Aplicativo Android",text:"Instale o Trafinexo como aplicativo no Android. A mesma experiência responsiva fica disponível na tela inicial."}
];
...
export default function ProductGuide(){
 ...
 useEffect(()=>{const seen=localStorage.getItem("trafinexo_product_guide_v2");if(!seen){const t=setTimeout(()=>setOpen(true),800);return()=>clearTimeout(t)}},[]);
 useEffect(()=>{
  if(!open)return;
 ...
 function close(){localStorage.setItem("trafinexo_product_guide_v2","1");setOpen(false);setIndex(0)}
 function next(){if(index>=steps.length-1)close();else setIndex(index+1)}
 return <>
  <button className="product-guide-trigger" onClick={()=>{setIndex(0);setOpen(true)}}>Guia</button>
