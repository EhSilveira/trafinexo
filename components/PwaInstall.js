...
export default function PwaInstall(){
  ...
  async function install(){
    ...
    setHint("No Android, abra o menu ⋮ do Chrome e toque em ‘Instalar app’ ou ‘Adicionar à tela inicial’. O Trafinexo funciona como PWA, sem depender da Play Store.");
  }
  if(installed)return null;
  return <div className="infotec-pwa-install" role="region" aria-label="Baixar aplicativo Android">
    <button onClick={install} type="button">📲 Baixar app Android</button>
    {hint&&<span>{hint}</span>}
    <style jsx>{`
      .infotec-pwa-install{position:fixed;right:16px;bottom:18px;z-index:9998;display:flex;max-width:min(380px,calc(100vw - 32px));flex-direction:column;align-items:flex-end;gap:7px;padding-bottom:env(safe-area-inset-bottom)}
